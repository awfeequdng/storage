...
void IniReader::SetKey(string field, string key, string value, bool isSystemParam, size_t lineNumber)
{
...
//					DEBUG("Truncating line at comment"<<line[commentIndex-1]);
            ...
            if ((equalsIndex = line.find_first_of("=")) == string::npos)
            {
                ...
                {
                    ERROR("Malformed Line "<<lineNumber<<" (missing equals or '[' and ']')");
                    abort();
                }
                else if( leftBracketIndex  != string::npos&&
                         rightBracketIndex != string::npos )
                {
                    field = line.substr(leftBracketIndex+1,rightBracketIndex-leftBracketIndex);
                    continue;
                }
			}
            ...
            if(!field.empty())
            {
                IniReader::SetKey(field,key, valueString, isSystemFile, lineNumber);
            }
